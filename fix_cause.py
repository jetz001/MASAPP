...
for rca_id, root_cause, failure_type in rows:
    ...
    # Simple heuristics to make data look realistic
    if 'pm' in ft or 'บำรุงรักษา' in ft:
        new_cause = 'เสื่อมสภาพตามอายุ (Normal Wear & Tear)'
    ...
    # If it's still wear & tear, randomly assign some to others to show graph variation 
    # based on the rca_id string hash so it's deterministic
...
conn.commit()
print(f'Updated {len(updates)} records for root cause')
