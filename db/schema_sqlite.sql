...
-- =============================================================================
-- MODULE WORK PROCESSES & LEAN ANALYSIS: Work Process Flow Chart & Steps
-- =============================================================================
CREATE TABLE IF NOT EXISTS work_processes (
  ...
  factory           TEXT,
  department        TEXT,
  method_type       TEXT NOT NULL DEFAULT 'current', -- 'current' (ปัจจุบัน) | 'improved' (ปรับปรุง)
  parent_process_id TEXT REFERENCES work_processes(process_id) ON DELETE SET NULL,
  work_type         TEXT NOT NULL DEFAULT 'man',     -- 'man' (คน) | 'product' (ผลิตภัณฑ์)
  ...
  approved_date     TEXT,
  notes             TEXT,
  status            TEXT NOT NULL DEFAULT 'draft',   -- 'draft' | 'active' | 'implemented'
  created_at        DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at        DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS work_process_steps (
  ...
  description      TEXT NOT NULL,
  event_type       TEXT NOT NULL, -- 'operation', 'transportation', 'inspection', 'delay', 'storage'
  distance_meters  REAL NOT NULL DEFAULT 0.0,
  parts_quantity   TEXT,
  tools_used       TEXT,
  duration_minutes REAL NOT NULL DEFAULT 0.0,
  value_type       TEXT NOT NULL, -- 'va' (จำเป็น), 'nva' (สูญเปล่า), 'nnva' (สูญเปล่าแต่จำเป็น)
  problem_cause    TEXT,
